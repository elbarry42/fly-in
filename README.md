...
| `priority`   |             1 | Preferred by pathfinding when appropriate |
...
* path length;
* estimated connection congestion.

The available paths are then distributed among the drones so that drones are not unnecessarily concentrated on the same route.
...
The path-allocation stage also evaluates candidate paths according to their cost and estimated congestion. This improves route distribution but adds additional computation compared with selecting one shortest path for every drone.
